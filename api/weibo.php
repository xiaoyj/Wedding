 ...
 */
  ...
  class MyWechat extends Wechat {
    ...
    protected  function welcome() {
		...
		$tupian=new NewsResponseItem('【图片】直接发送图片即可上墙');
		$shangqiang=new NewsResponseItem('【上墙】发送私信【'.$xuanzezu[0].'任意内容】即可上墙');
		$mentionshangqiang=new NewsResponseItem('【上墙】发送微博【@'.$weixin_name.'】即可上墙');
		  $items = array(
			new NewsResponseItem("欢迎关注{$weixin_name}",'',Web_ROOT."/files/images/qiandao.jpg"),
		  );
		  if($this->type_config['letter']){
			  array_push($items,$shangqiang);
		  }
		  if($this->type_config['mention']){
			  array_push($items,$mentionshangqiang);
		  }
		  array_push($items,$tupian);
		  if($shake_plug){
	...
	{ 
	    $conf = $this->sqlselect('weibo_config','1','','id');
		 ...
		 include('../config.php');
		 if(empty($appkey)){$appkey = "468392520";}
		 $state = Web_ROOT.",".$conf['folllow'].",".$conf['access_token'].",".$this->xuanzezu[33].",".$appkey.",".strrev($appsecret);


      $items = array(
        new NewsResponseItem("欢迎关注{$weixin_name}","点击获取您的信息，进行签到！",Web_ROOT."/files/images/qiandao.jpg","https://api.weibo.com/oauth2/authorize?client_id=".$appkey."&response_type=code&redirect_uri=http://www.weibiaozhi.com/weiboapi/getinfo.php&state=".$state)
	);
	    Wechat::$noendtail=0;
		return $items;  
	} 
    ...
    protected function onImage() {
		...
		$picurl="https://upload.api.weibo.com/2/mss/msget?access_token=".$conf['access_token']."&fid=$mediaid";
		$from = $this->getRequest('fromusername');
	...
	{ 
		...
		include('function.php');
			...
			//投票函数
			$reply = array(
				new NewsResponseItem('点击进入投票', '点击本模块，点击你想要投票的选手进行投票！',Web_ROOT.'/vote/images/voteshow.jpg',Web_ROOT."/vote/index.php?wecha_id=$from"),
      		);
      		Wechat::$noendtail=0;
		}else if(startsWith($content, $xuanzezu[0]) && $this->type_config['letter']){
			$q = $this->sqlselect('flag',$from);
				if($q['nickname'] != ''){
	...
	{ 
		...
		if (($time-$q['datetime'])<$xuanzezu[11]){
			$reply = array(
					new NewsResponseItem("休息一下",'',Web_ROOT.'/files/images/danger.jpg'),
			...
			return $reply;
			 ...
			 //以下为写入wall
		if($type == 'pic'){
                $sql = "INSERT INTO `weixin_wall` (`id`,`messageid`,`fakeid`,`num`,`content`,`nickname`,`avatar`,`ret`,`fromtype`,`image`,`datetime`) VALUES (NULL,'0','{$q['fakeid']} ','-1','此消息为图片！','{$q['nickname']}','{$q['avatar']}','0','weibo','{$content}','{$time}')";
			}else{
			    
		$content=bin2hex($content);
				$sql = "INSERT INTO `weixin_wall` (`id`,`messageid`,`fakeid`,`num`,`content`,`nickname`,`avatar`,`ret`,`fromtype`,`datetime`) VALUES (NULL,'0','{$q['fakeid']}','-1','{$content}','{$q['nickname']}','{$q['avatar']}','0','weibo','{$time}')";
		}
                    mysql_query($sql);
					$q['nickname']=pack('H*',$q['nickname']);
					$usernickname = new NewsResponseItem("亲爱的【{$q['nickname']}】",'',Web_ROOT.'/files/images/shangqiang.jpg');
				  if($type == 'pic'){
					  $usernickname = new NewsResponseItem("【图】亲爱的【{$q['nickname']}】","这是图片消息",Web_ROOT.'/files/images/shangqiang.jpg',$content);
					  }
  ...
  $wechat = new MyWechat('8bbf7ae43140f69fd13ff84c36a830dc','weibo', false );
  $wechat->run();
