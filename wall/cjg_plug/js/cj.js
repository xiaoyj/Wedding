  $(function(){
	...
	function cj_reset() {
		...
		cjstart_btn='';
		$('.lotteryUserNum').html('<img src="cjg_plug/images/loading.gif" />');
		$('.winUserNum').html('<img src="cjg_plug/images/loading.gif" />');
		  $(".lotteryImg").attr("src","cjg_plug/images/pair-default.jpg");
		  $(".lotteryName").html("... ...");
		  $('.lott-wt').html("第"+cj_shady+"次抽奖现场");
		   cj_per= new Array(); 
			$.getJSON('cjg_plug/cj_data.php?action=ready&shady='+cj_shady,function(json){
			if(json){
				 $.each(json, function(i,v){
					cj_per.push(new Array(v['id'],v['avatar'],v['nickname'],v['shady'],v['from']));
				});
	...
	function cj_start(){
	    ...
	    if(cj_retime == '0'){   
			...
			if(len>0){
				for(i=0;i<len;i++){
					if(cj_per[i][3] == cj_shady){
						cj_shadyper[0] = cj_per[i][0];
						cj_shadyper[1] = cj_per[i][2];
						cj_shadyper[2] = cj_per[i][1];
						cj_shadyper[3] = i;
						}
					};
				cj_gogo = setInterval(function(){
                  ...
                  $("#fromtype").attr("src","images/ico-"+cj_per[num][4]+".png");
	...
	function cj_stop(){
		...
		if(cj_shadyper[0]){
                  $(".lotteryImg").attr("src",cj_shadyper[2]);
                  $(".lotteryName").html(cj_shadyper[1]);
                  $("#cj_mid").val(cj_shadyper[0]); 
				  $("#cj_mid").attr('name',cj_shadyper[3]);
					cj_shadyper= new Array();
			}
			  ...
			  cj_id = cj_id + 1;
				...
				cjstart_btn ='';
			  $.post("cjg_plug/cj_data.php?action=ok",{id:cj_mid,grade:cj_shady},function(msg){
				  if(msg==1){
					cj_jadge = 1;
	...
	$('#cj_resetBtn').click(function(){
       if(confirm("确定重置摇奖池吗？")){
				...
				clearInterval(cj_gogo);
				cjstart_btn.children("span").text("正在准备数据…");
			  $.post("cjg_plug/cj_data.php?action=reset",{},function(msg){
				  if(msg==2){
					cj_jadge = 1;
					cj_id = 0;
					$(".winUserList").empty();
